; ModuleID = '/tmp/tmp.KLDxEo7phM/reduced.bc'
; LLVM ERROR: unable to legalize instruction: %85:vid(<32 x s1>) = G_FCMP floatpred(oeq), %83:vfid(<32 x s16>), %81:vfid (in function: __cxa_deleted_virtual
...
@G.1 = addrspace(1) global <32 x i1> splat (i1 true)

define spir_func void @__cxa_deleted_virtual() addrspace(4) {
entry:
  %C = fcmp oeq <32 x half> splat (half 0xH3C00), zeroinitializer
  store <32 x i1> %C, ptr addrspace(1) null, align 4
  unreachable
}
